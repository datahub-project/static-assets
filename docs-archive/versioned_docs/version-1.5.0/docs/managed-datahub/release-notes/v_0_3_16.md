---
title: v0.3.16
slug: /managed-datahub/release-notes/v_0_3_16
custom_edit_url: >-
  https://github.com/datahub-project/datahub/blob/master/docs/managed-datahub/release-notes/v_0_3_16.md
---
# v0.3.16
...
- ES/OpenSearch: set `dynamic: true` on object-type mappings to avoid reindex loops (e.g. ownerTypes).
- Index build: configurable slow-op timeout, count retry, and optional createIndex retry; long timeout used for count/refresh/reindex/listTasks/etc.
- Search engine detection: failure reasons included in thrown IOException; null-safe version checks.
- ES8 reindex uses `waitForCompletion(false)` for async reindex.
- Retention: per-(urn,aspect) DELETE for better index use; version history only when policy allows; retention ingestion moved to datahub-upgrade non-blocking step.

#### Ingestion

- metadata-ingestion: setuptools &lt;82, urllib3 1.26.x, Snowflake deps updated (connector ≥4, cryptography≥46), added gcs-slim/abs-slim.
- `avro_schema_to_mce_fields` accepts optional `validate_names` for avro parse.
- Integrations Snowflake: `private_key` supports both SecretStr and str.
- Pinned `cryptography>=46.0.5` in ingestion for CVE-2026-26007.

#### Product

- “Entities per platform” chart excludes query index so query entities don’t inflate counts.
- OIDC settings: defaults in DEFAULT_SETTINGS; inputs show empty + placeholder so saved vs default is clear.
- Search bar keeps query and suggestions after selecting an entity (`onSearch`, no onClear).
- Document editor: theme-aware toolbar; preventDefault on toolbar so dropdowns don’t steal focus; click-outside with `.ant-dropdown` ignored.
- Confluence recipe: config paths updated to `spaces.allow/deny` and `pages.allow/deny`.
- Slack/Teams chat excludes USER- and MUTATION-tagged tools.
- Dark/light theme tokens extended (e.g. textOnFill*, textOnSurface*, red400/500) for toolbar/editor.

### v0.3.16.4-acryl

- Various small UX fixes (data product proposals, users in queries table, fetching forms data in the task center)
- Fixes for server-side Semantic Search Configs for bedrock in GraphQL
- Support for injecting custom headers when forwarding LLM calls Open AI Proxy
- Cleanup extraneous logs related to OpenSearch rescoring
...
- **Ask DataHub (Public Beta)** - Ask DataHub chat assistant is now in Public Beta! Available in DataHub, Slack, & Microsoft Teams. This means it's available for all customers to use. If you do NOT want Ask DataHub enabled for your instance, please reach out to your DataHub customer representative. Learn more about Ask DataHub [here](/docs/features/feature-guides/ask-datahub).
